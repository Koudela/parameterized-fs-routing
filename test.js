...
test.serial('set up with plain node', async t => {
...
test.serial('set up with express', async t => {
...
test.serial('matching controller functions', async t => {
...
test.serial('traversing the controller-pipe', async t => {
    t.is(await doRequest('GET', '/traversing%20the%20controller-pipe/1/2/3'), 'start_stop!')
    t.is(await doRequest('GET', '/traversing%20the%20controller-pipe/foo/bar'), 'start_stop!')
})

test.serial('the controller function call', async t => {
    t.is(
        await doRequest('GET', '/the+controller+function+call'),
        'other_{"name":"get.js","path":"./testcases/routing-root/the+controller+function+call/get.js","params":[],"args":{}}_[{"name":"index.js","path":"./testcases/routing-root/index.js","params":["the+controller+function+call"],"args":{}},{"name":"get.js","path":"./testcases/routing-root/the+controller+function+call/get.js","params":[],"args":{}},{"name":"index.js","path":"./testcases/second-root/index.js","params":["the+controller+function+call"],"args":{}}]'
    )
})

test.serial('example controller', async t => {
...
test.serial('differences in prod and env environment (prod)', async t => {
    process.env.NODE_ENV = 'production'

    const server = await setUpExpress(3002)
...
test.serial('differences in prod and env environment (dev)', async t => {
    process.env.NODE_ENV = 'dev'

    const server = await setUpExpress(3003)

    t.is(await doRequest('GET', '/prod-vs-dev-env', '', 3003), 'second-root_!')
    ...
    t.is(await doRequest('GET', '/prod-vs-dev-env', '', 3003), 'old no result!')
